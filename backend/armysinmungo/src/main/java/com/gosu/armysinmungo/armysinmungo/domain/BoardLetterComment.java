...
@Entity
@Table(name = "BOARD_LETTER_COMMENT")
@Builder @Getter
@AllArgsConstructor(access = AccessLevel.PRIVATE)
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@EntityListeners(AuditingEntityListener.class)
public class BoardLetterComment {
    ...
    @ManyToOne
    @JoinColumn(name = "BOARD_LETTER_POST_ID")
    private BoardLetterPost boardLetterPost;

    public BoardCommentResponse toBoardCommentResponse() {
        return BoardCommentResponse.builder()
            .postNum(boardLetterPost.getPostNum())
